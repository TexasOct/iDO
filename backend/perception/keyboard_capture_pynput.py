...
注意: 此实现不适用于 macOS，因为 pynput 在 macOS 上与 Tauri 主线程冲突
"""
...
class KeyboardCapturePynput(BaseCapture):
    """键盘事件捕获器（pynput 实现）"""
        ...
        if self.listener:
            try:
                ...
                if hasattr(self.listener, '_thread') and self.listener._thread:
                    logger.debug(f"等待键盘监听线程结束...")
                    self.listener._thread.join(timeout=3.0)
                    if self.listener._thread.is_alive():
                        logger.warning("键盘监听线程未能在超时内结束，但继续进行")
                self.listener = None
                logger.info("键盘监听已停止（pynput）")
            except Exception as e:
                logger.error(f"停止键盘监听失败: {e}")
                self.listener = None
