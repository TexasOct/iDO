...
TODO: 未来可以考虑使用 Windows API (pywin32) 实现更原生的监控
"""
...
class WindowsKeyboardMonitor(BaseKeyboardMonitor):
    """Windows 键盘事件捕获器（使用 pynput）"""
    ...
    def start(self):
        """启动键盘监听"""
        if self.is_running:
            logger.warning("Windows 键盘监听已在运行中")
            return
        ...
        try:
            ...
            self.listener.start()
            logger.info("✅ Windows 键盘监听已启动（使用 pynput）")

        except Exception as e:
            self.is_running = False
            logger.error(f"启动 Windows 键盘监听失败: {e}")
            raise
    ...
    def stop(self):
        ...
        if self.listener:
            try:
                ...
                self.listener = None
                logger.info("Windows 键盘监听已停止")
            except Exception as e:
                logger.error(f"停止 Windows 键盘监听失败: {e}")
