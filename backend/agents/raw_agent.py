...
class RawAgent:
    ...
    def __init__(self, max_screenshots: int = 20):
        ...
        try:
            ...
            logger.debug("RawAgent: Image optimization enabled")
        except Exception as exc:
            logger.warning(
                f"RawAgent: Failed to initialize image optimization, will skip compression: {exc}"
            )
            self.image_optimizer = None
    ...
    def _optimize_image_base64(self, base64_data: str, *, is_first: bool) -> str:
        ...
        try:
            ...
            if optimized_bytes and optimized_bytes != img_bytes:
                logger.debug(
                    "RawAgent: Image compression completed "
