...
def test_importance_analyzer():
    ...
    success = (
        results[0][1] == 'low' and
        results[1][1] in ['medium', 'high'] and
        results[2][1] == 'high'
    )
...
def test_dynamic_compressor():
    ...
    for level in levels:
        ...
        print(f"{level:12s}: {compressed_size/1024:.1f} KB "
              f"({ratio*100:.1f}%), "
              f"质量={meta['quality']}, "
    ...
    success = (
        results[0]['ratio'] < results[1]['ratio'] <
        results[2]['ratio'] < results[3]['ratio']
    )
...
def test_region_cropper():
    ...
    success = (
        not meta1['is_cropped'] and
        meta2.get('is_cropped', False) and
        not meta3['is_cropped']
    )
...
def test_advanced_optimizer():
    ...
    for config_name, compression_level, enable_cropping in configs:
        ...
        print(f"    Token 节省: {stats['tokens']['saved']} ({stats['tokens']['reduction_percentage']:.1f}%)")
