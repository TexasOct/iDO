...
本文档详细说明了 Rewind 后端系统的架构设计、数据流、以及各个关键组件的实现。
...
Rewind 后端采用 **三层分层架构**，数据从底层的原始事件逐层处理和提炼，最终为用户提供智能化的任务建议。
...
Rewind 使用**通用 API Handler 系统**，允许一次定义的 API 接口在 **PyTauri** 和 **FastAPI** 上都自动可用，无需重复代码。
...
# 在 src-tauri/python/rewind_app/__init__.py 中注册
