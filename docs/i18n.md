# 国际化 (i18n) 指南

本文档说明了 Rewind 项目的国际化系统、如何添加新语言、以及如何在组件中使用翻译。

## 目录

- [概览](#概览)
- [项目配置](#项目配置)
- [添加翻译](#添加翻译)
- [在组件中使用](#在组件中使用)
- [验证翻译](#验证翻译)
- [最佳实践](#最佳实践)

## 概览

Rewind 使用**类型安全的 TypeScript 首先 i18n 系统**，具有以下特点：

- ✅ **完全类型安全**：所有翻译键在编译时检查
- ✅ **无运行时开销**：翻译字符串直接编译到代码中
- ✅ **自动验证**：确保所有语言有一致的键
- ✅ **易于维护**：英文文件是源文件，其他语言与之同步

### 工作流程

```
src/locales/en.ts (English - Source)
         ↓
    定义所有键和类型
         ↓
pnpm check-i18n 验证
         ↓
src/locales/zh-CN.ts (Chinese)
src/locales/ja-JP.ts (Japanese - 可添加)
         ↓
    在组件中使用
```

## 项目配置

### 文件结构

```
src/locales/
├── en.ts                # 英文（源文件，定义结构）
├── zh-CN.ts            # 中文简体
└── index.ts            # 导出
```

### 翻译文件格式

#### 英文源文件（en.ts）

```typescript
// src/locales/en.ts
export const en = {
  // 菜单和导航
  sidebar: {
    activity: 'Activity',
    dashboard: 'Dashboard',
    agents: 'Agents',
    settings: 'Settings',
  },

  // 活动功能
  activity: {
    title: 'Activity Timeline',
    description: 'View your work activities',
    empty: 'No activities yet',
    filter: 'Filter activities',
    search: 'Search',
  },

  // Agent 功能
  agents: {
    title: 'Tasks',
    noTasks: 'No tasks yet',
    createTask: 'Create Task',
    status: {
      todo: 'To Do',
      doing: 'In Progress',
      done: 'Done',
      cancelled: 'Cancelled',
    },
  },

  // 设置页面
  settings: {
    title: 'Settings',
    general: 'General Settings',
    llm: 'LLM Configuration',
    apiKey: 'API Key',
    model: 'Model',
    language: 'Language',
    theme: 'Theme',
  },

  // 通用错误和消息
  common: {
    save: 'Save',
    cancel: 'Cancel',
    delete: 'Delete',
    edit: 'Edit',
    loading: 'Loading...',
    error: 'Error',
    success: 'Success',
  },

  errors: {
    loadFailed: 'Failed to load data',
    saveFailed: 'Failed to save changes',
    connectionError: 'Connection error',
    invalidInput: 'Invalid input',
  },
} as const
```

#### 中文翻译（zh-CN.ts）

```typescript
// src/locales/zh-CN.ts
export const zhCN = {
  // 菜单和导航
  sidebar: {
    activity: '活动',
    dashboard: '仪表板',
    agents: '任务',
    settings: '设置',
  },

  // 活动功能
  activity: {
    title: '活动时间线',
    description: '查看你的工作活动',
    empty: '暂无活动',
    filter: '筛选活动',
    search: '搜索',
  },

  // Agent 功能
  agents: {
    title: '任务',
    noTasks: '暂无任务',
    createTask: '创建任务',
    status: {
      todo: '待做',
      doing: '进行中',
      done: '已完成',
      cancelled: '已取消',
    },
  },

  // 设置页面
  settings: {
    title: '设置',
    general: '通用设置',
    llm: 'LLM 配置',
    apiKey: 'API Key',
    model: '模型',
    language: '语言',
    theme: '主题',
  },

  // 通用错误和消息
  common: {
    save: '保存',
    cancel: '取消',
    delete: '删除',
    edit: '编辑',
    loading: '加载中...',
    error: '错误',
    success: '成功',
  },

  errors: {
    loadFailed: '加载数据失败',
    saveFailed: '保存更改失败',
    connectionError: '连接错误',
    invalidInput: '输入无效',
  },
}
```

#### 导出文件（index.ts）

```typescript
// src/locales/index.ts
import { en } from './en'
import { zhCN } from './zh-CN'

export const locales = {
  en,
  'zh-CN': zhCN,
}

export type Locale = keyof typeof locales
export type TranslationKey = keyof typeof en
```

## 添加翻译

### 步骤 1：添加英文键

编辑 `src/locales/en.ts`，在适当的位置添加新键：

```typescript
export const en = {
  myFeature: {
    title: 'My Feature',
    description: 'Feature description',
    button: 'Click me',
  },
} as const
```

### 步骤 2：添加对应的中文翻译

编辑 `src/locales/zh-CN.ts`，**必须保持键的结构和名称完全相同**：

```typescript
export const zhCN = {
  myFeature: {
    title: '我的功能',
    description: '功能描述',
    button: '点击我',
  },
}
```

### 步骤 3：验证翻译

```bash
# 检查所有语言是否有一致的键
pnpm check-i18n
```

如果有遗漏，会看到类似的错误：

```
❌ Missing in zh-CN.ts:
  - myFeature.title
  - myFeature.button

❌ Extra keys in zh-CN.ts:
  - oldFeature.title
```

### 步骤 4：修复错误

根据错误提示，添加遗漏的翻译或删除多余的翻译。

## 在组件中使用

### 基础使用

使用 `useTranslation` hook 获取翻译函数：

```typescript
import { useTranslation } from '@/hooks/useTranslation'

export function MyComponent() {
  const { t } = useTranslation()

  return (
    <div>
      <h1>{t('myFeature.title')}</h1>
      <p>{t('myFeature.description')}</p>
      <button>{t('myFeature.button')}</button>
    </div>
  )
}
```

### 完整的组件示例

```typescript
// src/views/Settings/index.tsx
import { useTranslation } from '@/hooks/useTranslation'
import { useSettingsStore } from '@/lib/stores'

export function SettingsView() {
  const { t } = useTranslation()
  const language = useSettingsStore((state) => state.language)
  const updateLanguage = useSettingsStore((state) => state.updateLanguage)

  return (
    <div>
      <h1>{t('settings.title')}</h1>

      <section>
        <h2>{t('settings.general')}</h2>
        <label>{t('settings.language')}</label>
        <select value={language} onChange={(e) => updateLanguage(e.target.value)}>
          <option value="en">English</option>
          <option value="zh-CN">简体中文</option>
        </select>
      </section>

      <section>
        <h2>{t('settings.llm')}</h2>
        <label>{t('settings.apiKey')}</label>
        <input type="password" placeholder={t('settings.apiKey')} />
      </section>
    </div>
  )
}
```

### 动态翻译

某些情况下需要动态生成翻译键：

```typescript
import { locales } from '@/locales'

export function TaskStatusBadge({ status }: { status: 'todo' | 'doing' | 'done' }) {
  const { t } = useTranslation()

  // ✅ 安全的动态翻译
  const key = `agents.status.${status}`
  const translated = t(key as any) // 注意：需要类型断言

  return <span>{translated}</span>
}

// 或者预先定义所有状态
const statusLabels = {
  todo: 'agents.status.todo',
  doing: 'agents.status.doing',
  done: 'agents.status.done',
} as const

export function TaskStatusBadge({ status }) {
  const { t } = useTranslation()
  return <span>{t(statusLabels[status])}</span>
}
```

### 在属性中使用

```typescript
export function Button() {
  const { t } = useTranslation()

  return (
    <button
      title={t('common.save')}
      placeholder={t('activity.search')}
      aria-label={t('sidebar.activity')}
    >
      {t('common.save')}
    </button>
  )
}
```

## 验证翻译

### 自动验证

```bash
# 检查所有翻译的完整性和一致性
pnpm check-i18n
```

### 验证脚本原理

验证脚本检查：
1. ✅ 所有语言都有英文文件的所有键
2. ✅ 没有多余的键（在英文中不存在）
3. ✅ 键的嵌套结构完全相同

### 常见验证错误

#### 错误 1：遗漏翻译

```
❌ Missing in zh-CN.ts:
  - activity.newField
```

**解决：** 在 `zh-CN.ts` 中添加遗漏的键

```typescript
activity: {
  newField: '新字段',
}
```

#### 错误 2：多余的键

```
❌ Extra keys in zh-CN.ts:
  - oldFeature.title
```

**解决：** 删除在英文文件中不存在的键

```typescript
// 删除这一行
// oldFeature: { title: '...' }
```

#### 错误 3：嵌套结构不同

```
❌ Structure mismatch at settings:
  English: { llm: { apiKey: '...' } }
  zh-CN: { llmConfig: { apiKey: '...' } }
```

**解决：** 确保键的名称完全相同

```typescript
// ❌ 错误
settings: {
  llmConfig: { ... }
}

// ✅ 正确
settings: {
  llm: { ... }
}
```

## 最佳实践

### ✅ 组织结构

按功能模块组织翻译键：

```typescript
export const en = {
  // 导航和菜单
  sidebar: { ... },
  navigation: { ... },

  // 功能模块
  activity: { ... },
  agents: { ... },
  dashboard: { ... },
  settings: { ... },

  // 通用
  common: { ... },
  errors: { ... },
}
```

### ✅ 命名规范

- 使用小驼峰命名：`myFeatureName`
- 嵌套的键用点号分隔：`activity.emptyState.message`
- 尽可能简洁和自描述

### ✅ 避免硬编码

❌ **不好**：
```typescript
<h1>Activity Timeline</h1>
```

✅ **好**：
```typescript
const { t } = useTranslation()
<h1>{t('activity.title')}</h1>
```

### ✅ 长文本处理

对于长的描述或帮助文本，可以使用多行字符串：

```typescript
export const en = {
  help: {
    apiKey: `Enter your OpenAI API key here.
You can find it in your account settings.
This is required to enable AI features.`,
  },
}
```

### ✅ 保留特殊字符

如果翻译中包含特殊字符，使用转义：

```typescript
export const en = {
  special: {
    copyright: '© 2024 Texas',
    trademark: '™',
    quote: '"Hello World"',
  },
}
```

### ✅ 日期和数字格式

使用 `date-fns` 和 `Intl` API 来格式化日期和数字：

```typescript
import { format } from 'date-fns'
import { zhCN as dateZhCN } from 'date-fns/locale'

function formatDate(date: Date, locale: string) {
  const localeMap = {
    en: undefined,
    'zh-CN': dateZhCN,
  }

  return format(date, 'PPP p', { locale: localeMap[locale] })
}
```

## 添加新语言

### 步骤 1：创建新的语言文件

```typescript
// src/locales/ja-JP.ts
export const jaJP = {
  sidebar: {
    activity: 'アクティビティ',
    dashboard: 'ダッシュボード',
    agents: 'タスク',
    settings: '設定',
  },
  // ... 完整内容
}
```

### 步骤 2：更新导出

```typescript
// src/locales/index.ts
import { en } from './en'
import { zhCN } from './zh-CN'
import { jaJP } from './ja-JP'

export const locales = {
  en,
  'zh-CN': zhCN,
  'ja-JP': jaJP,
}
```

### 步骤 3：验证

```bash
pnpm check-i18n
```

### 步骤 4：更新设置

在设置页面添加新语言选项：

```typescript
// src/components/Settings/LanguageSelect.tsx
<select>
  <option value="en">English</option>
  <option value="zh-CN">简体中文</option>
  <option value="ja-JP">日本語</option>
</select>
```

## 获取帮助

- 📖 查看 [开发指南](./development.md)
- 🐛 报告 Bug：[GitHub Issues](https://github.com/TexasOct/Rewind/issues)
