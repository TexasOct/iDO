...
- Main code locations: `src-tauri/python/rewind_app/` and symlinked `backend/`
- PyTauri automatically generates TypeScript client in `src/lib/client/` (DO NOT manually edit)
- Communication: Frontend → PyTauri Client → Rust → Python

#### FastAPI Server (Testing/Development)

The backend also provides a standalone FastAPI server for testing without the Tauri desktop wrapper:
...
See [docs/fastapi_usage.md](docs/fastapi_usage.md) for detailed FastAPI documentation.
...
- `docs/fastapi_usage.md` - FastAPI standalone server usage guide
...
- `app.py` - **NEW**: FastAPI application entry point (standalone server)
