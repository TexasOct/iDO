...
>
> - Always run `uv run ty check` before submitting backend changes.
...
# Production: imports from rewind_backend/ (installed package)

# This is handled automatically in src-tauri/python/rewind_app/__init__.py
# Always use: from backend.X import Y (not rewind_backend)
```
...
- `src-tauri/python/rewind_app/__init__.py`: PyTauri entry point, registers commands
...
- SQLite database at `~/.config/rewind/rewind.db`
